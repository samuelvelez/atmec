<script type="text/javascript">
    ...
    let signals_list = {!! json_encode($signals) !!};
    let smarkers = new Map();

    let regulators_list = {!! json_encode($regulators) !!};
    let rmarkers = new Map();

    let devices_list = {!! json_encode($devices) !!};
    let dmarkers = new Map();

    let poles_list = {!! json_encode($poles) !!};
    let pmarkers = new Map();

    let tensors_list = {!! json_encode($tensors) !!};
    let tmarkers = new Map();

    let lights_list = {!! json_encode($lights) !!};
    let lmarkers = new Map();

    function show_map() {
        let LatitudeAndLongitude = new google.maps.LatLng({{ $latitude }}, {{ $longitude }});
    ...
    $.each(signals_list, function (index, item) {
        let marker = new google.maps.Marker({
            map: map,
            icon: {
                url: "{{ asset('images/icons/signal.png') }}",
                scaledSize: new google.maps.Size(30, 30)
            },
            //icon: "{{ asset('images/icons/signal.png') }}",
            title: 'Señal: ' + item.id + '',
    ...
    $.each(regulators_list, function (index, item) {
        let marker = new google.maps.Marker({
            map: map,
            icon: {
                url: "{{ asset('images/icons/regulator.png') }}",
                scaledSize: new google.maps.Size(30, 30)
            },
    ...
    $.each(devices_list, function (index, item) {
        let marker = new google.maps.Marker({
            map: map,
            icon: {
                url: "{{ asset('images/icons/device.png') }}",
                scaledSize: new google.maps.Size(30, 30)
            },
    ...
    $.each(poles_list, function (index, item) {
        let marker = new google.maps.Marker({
            map: map,
            icon: {
                url: "{{ asset('images/icons/pole.png') }}",
                scaledSize: new google.maps.Size(30, 30)
            },
            title: 'Poste: ' + item.id + '',
            position: new google.maps.LatLng(item.latitude, item.longitude),
    ...
    $.each(lights_list, function (index, item) {
        let marker = new google.maps.Marker({
            map: map,
            icon: {
                url: "{{ asset('images/icons/light.png') }}",
                scaledSize: new google.maps.Size(30, 30)
            },
