<script>
    $(function() {
        ...
        searchform.submit(function(e) {
            ...
            let noResulsHtml = '<tr>' +
                                '<td>{!! trans("regulator-devices.search.no-results") !!}</td>' +
                                '<td></td>' +
                                '<td></td>' +
                                '<td></td>' +
                                '<td></td>' +
            ...
            $.ajax({
                type:'POST',
                url: "{{ route('search-regulator-devices') }}",
                ...
                success: function (result) {
                    ...
                    if (jsonData.length !== 0) {
                        ...
                        $.each(jsonData, function(index, val) {
                            let showCellHtml = '<a class="btn btn-sm btn-success btn-block" href="regulator-devices/' + val.id + '" data-toggle="tooltip" title="{{ trans("regulator-devices.tooltips.show") }}">{!! trans("regulator-devices.buttons.show") !!}</a>';
                            let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="regulator-devices/' + val.id + '/edit" data-toggle="tooltip" title="{{ trans("regulator-devices.tooltips.edit") }}">{!! trans("regulator-devices.buttons.edit") !!}</a>';
                            let deleteCellHtml = '<form method="POST" action="/regulator-devices/'+ val.id +'" accept-charset="UTF-8" data-toggle="tooltip" title="Delete">' +
                                    ...
                                    '<button class="btn btn-danger btn-sm" type="button" style="width: 100%;" data-toggle="modal" data-target="#confirmDelete" data-title="Eliminar poste" data-message="¿Está seguro que desea eliminar el poste? ¡Eliminará con el todas sus dependencias!">' +
                                        '{!! trans("regulator-devices.buttons.delete") !!}' +
                                    '</button>' +
                                '</form>';
                    ...
                    trafficpolesCount.html(jsonData.length + " {!! trans('regulator-devices.search.found-footer') !!}");
                    intersectionPagination.hide();
                    cardTitle.html("{!! trans('regulator-devices.search.title') !!}");
                },
        ...
        searchformInput.keyup(function(event) {
            if ($('#regulator_device_search_box').val() !== '') {
                ...
                cardTitle.html("{!! trans('regulator-devices.showing-all-regulator-devices') !!}");
