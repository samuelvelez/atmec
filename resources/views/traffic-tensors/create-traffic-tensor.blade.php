...
@section('template_title')
    {!! trans('traffic-tensors.create-new-traffic-tensor') !!}
@endsection
        ...
        <div class="row">
            <div class="col-lg-10 offset-lg-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        {!! Form::open(array('route' => 'traffic-tensors.store', 'method' => 'POST', 'role' => 'form', 'class' => 'needs-validation')) !!}
 ...
 <div class="form-group has-feedback row {{ $errors->has('poles') ? ' has-error ' : '' }}">
                                    ...
                                    <select name="poles[]" id="poles">
                                        ...
                                        @if ($traffic_poles)
                                            @foreach($traffic_poles as $traffic_pole)
                                                <option value="{{ $traffic_pole->id }}" {{ old('poles') == $traffic_pole->id ? 'selected' : '' }}>{{ $traffic_pole->id }} | {{ $traffic_pole->intersection->main_st }} y {{ $traffic_pole->intersection->cross_st }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                       ...
                       <div class="form-group has-feedback row {{ $errors->has('material') ? ' has-error ' : '' }}">
                        ...
                        <div class="form-group has-feedback row {{ $errors->has('comment') ? ' has-error ' : '' }}">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                    {!! Form::textarea('comment', NULL, array('id' => 'comment', 'rows' => '3', 'class' => 'form-control', 'placeholder' => trans('forms.create_traffic_tensor_ph_comment'))) !!}
                                    <div class="input-group-append">
                        ...
                        {!! Form::button(trans('forms.create_traffic_tensor_button_text'), array('class' => 'btn btn-success margin-bottom-1 mb-1 float-right','type' => 'submit' )) !!}
                        {!! Form::close() !!}
                    </div>
