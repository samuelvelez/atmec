            ...
            <div class="col-lg-10 offset-lg-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        {!! csrf_field() !!}
                        <input type="hidden" name="esedicion" id="esedicion" value="Si">
                        <input type="hidden" name="quitados" id="quitados" value="">
                        ...
                        <div class="form-group has-feedback row {{ $errors->has('description') ? ' has-error ' : '' }}">
                            ...
                            <div class="col-md-9">
                                <div class="input-group">
                                    {!! Form::textarea('description',  $desc, array('id' => 'description', 'rows' => '3', 'class' => 'form-control', 'placeholder' => trans('materials.create_ph_description'))) !!}
                                </div>
                              ...
                              ?>
                                <div class="col-md-12">
                                    <table id="materials"
                                        ...
                                        <tbody>
                                                @if ($materialid)
                                                @foreach ($materialid as $materia)
                                                @foreach ($metrics as $metrica)
                                               <?php 
                                              $idmtr = $materia->id_matrepord;
                                              $fechacrea=$materia->date_create_ori;
                                              $iduserr=$materia->id_userrequire;
                                              $iduserc=$materia->id_usercreate;
                                              $dateaprobneg=$materia->date_aprob_or_neg;
                                              $iduseraporneg=$materia->id_useraproborneg;
                                              $reporid = $materia->report_id;
                                              
                                               if ($materia->metric_id == $metrica->id){
                                               $metricx[] = $metrica->abbreviation;
                                                $idproductx[] = $materia->material_id; 
                                               $amountx[] = $materia->amount;
                                               }
                                               ...
                                               <?php 
                                               if ($materia->material_id  == $material->id){
                                               $productx[]= $material->name;
                                               }
                                                       ?>
                                                @endforeach
                                                ...
                                                @endif
                                                <?php 
                                                       for ($i = 0; $i <count($amountx); $i++) {
                                                       echo '<tr><td>'.$idproductx[$i].'</td>
                                                           <td>'.$productx[$i].'</td><td>'.$metricx[$i].'</td>
                                                               <td>'.$amountx[$i].'</td></tr>';
                                                       }
                                                ?>
                                        </tbody>
...
<!--                                    <span>{{ $collectors }}</span>
  ...
  {!! Form::hidden("idmtr", $idmtr, array('id' => 'idmtr')) !!}
  {!! Form::hidden("iduserr", $iduserr, array('id' => 'iduserr')) !!}
  {!! Form::hidden("iduserc", $iduserc, array('id' => 'iduserc')) !!}
  {!! Form::hidden("fechac", $fechacrea, array('id' => 'fechac')) !!}
  {!! Form::hidden("dateaporne", $dateaprobneg, array('id' => 'dateaporne')) !!}
  {!! Form::hidden("iduseraon", $iduseraporneg, array('id' => 'iduseraon')) !!}
  {!! Form::hidden("repid", $reporid, array('id' => 'repid')) !!}
    ...
    <script>
        ...
        materials_tbl.row.add([
            ...
            $('#del-material').click(function () {
                var valor = '';
                var valor =$("tr.selected").text();
//                  valor = valor.split("A");valor = valor.split("J");valor = valor.split("S");
//                  valor = valor.split("B");valor = valor.split("K");valor = valor.split("T");
//                  valor = valor.split("C");valor = valor.split("L");valor = valor.split("U");
//                  valor = valor.split("D");valor = valor.split("M");valor = valor.split("V");
//                  valor = valor.split("E");valor = valor.split("N");valor = valor.split("W");
//                  valor = valor.split("F");valor = valor.split("O");valor = valor.split("X");
//                  valor = valor.split("G");valor = valor.split("P");valor = valor.split("Y");
//                  valor = valor.split("H");valor = valor.split("Q");valor = valor.split("Z");
//                  valor = valor.split("I");valor = valor.split("R");
                 valor = valor.split(" ");

                   //alert(value[0]);
                   if (contador==0){
                document.getElementById('quitados').value= valor[0];
            } else { 
                document.getElementById('quitados').value= document.getElementById('quitados').value +','+ valor[0];
            }
